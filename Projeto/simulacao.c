...
int estadoDia; //variavel para controlar a maquina de estado da animacao do dia
float rotacaoLuzes; //variavel para controlar a rotacao das luzes em torno da cena
float distanciaLuzes; //variavel para definir o raio de rotacao das luzes
float timeScale; //variavel para controlar a quantidade de quadros por segundo
...
void animacaoLuz(){
	...
	if(estadoDia == 2){ //anoitecer
		if(G_Luzes[0].difusa[0] >= porDoSol[0]){ //se a cor ja tiver chegado no por do sol
			for(i=0; i<3; i++){
				G_Luzes[0].difusa[i] += -(amanhecer[i] - porDoSol[i]) / (timeScale*5.0); //ao longo de 5 segundos, a cor se aproximara da desejada
			}
		}

		G_Luzes[0].pos.x = cos(rotacaoLuzes) * distanciaLuzes; //gira as luzes em torno do centro
		G_Luzes[0].pos.y = sin(rotacaoLuzes) * distanciaLuzes;

		G_Luzes[1].pos.x = cos(rotacaoLuzes - PI) * distanciaLuzes;
		G_Luzes[1].pos.y = sin(rotacaoLuzes - PI) * distanciaLuzes;

		rotacaoLuzes+= PI/(10*timeScale); //a luz leva 10 segundos para girar ate o lado oposto 
		if(G_Luzes[0].pos.y < 0 && G_Luzes[0].enabled){
			G_Luzes[0].enabled = 0;
			for(i=0; i<3; i++){
				G_Luzes[0].difusa[i] = -1.0;
			}
			glDisable(GL_LIGHT0);
		}
		...
		if(rotacaoLuzes >= 3*PI/2){ //se o sol completamente se pos (pi/2 -> 3pi/2), entao o anoiteceu aconteceu
			rotacaoLuzes = -PI/2; //inverte para -pi/2, para facilitar os calculos
			estadoDia = -1;
			printf("Anoitecimento completo\n");
		}
	}

	if(estadoDia == -2){ //amanhecer
			if(G_Luzes[0].difusa[0] <= amanhecer[0]){ //comecar a trocar sua cor
				for(i=0; i<3; i++){
					G_Luzes[0].difusa[i] += -(-1.0 - amanhecer[i]) / (timeScale*10.0); //ao longo de 5 segundos, a cor se aproximara da desejada
				}
			}
		G_Luzes[0].pos.x = cos(rotacaoLuzes) * distanciaLuzes;
		G_Luzes[0].pos.y = sin(rotacaoLuzes) * distanciaLuzes;

		G_Luzes[1].pos.x = cos(rotacaoLuzes - PI) * distanciaLuzes;
		G_Luzes[1].pos.y = sin(rotacaoLuzes - PI) * distanciaLuzes;

		rotacaoLuzes+= PI/(10*timeScale); //a luz leva 10 segundos para descer ate o lado oposto 
		if(G_Luzes[1].pos.y < 0 && G_Luzes[1].enabled){
			G_Luzes[1].enabled = 0;
			glDisable(GL_LIGHT1);
		}
		...
		if(rotacaoLuzes >= PI/2){ //se o sol chegou ao pico, o dia completamente nasceu
			estadoDia = 1;
...
void atualizarPosicoes(int a){
   ...
   vector3 aux;
   for(i=0; i<3; i++){
   	aux = escalarVetor(-G_Carros[0]->veloc/timeScale, G_Carros[0]->direc); //calcula movimento relativo ao carro principal
   ...
   for(i=1; i<10; i++){
      ...
      aux = escalarVetor(-G_Carros[0]->veloc/timeScale, G_Carros[0]->direc); //calcula o movimento relativo do carro principal
      G_Carros[i]->pos = somarVetores(G_Carros[i]->pos, aux);  //movimenta com base nisso
      aux = escalarVetor(G_Carros[i]->veloc/timeScale, G_Carros[i]->direc); //calcula o movimento com base na propria velocidade
...
void simulacao(void){
   ...
   glutSwapBuffers();
   glutTimerFunc(1000/timeScale, atualizarPosicoes, 1); //chama a funcao de animacao a cada 33milisegundos, equivalendo a aproximadamente 30 quadros por segundo
}
